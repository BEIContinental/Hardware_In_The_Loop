...
#include "testcode.h"                  /* Model's header file */
#include "saveData.h"
#include "rtwtypes.h"
...
void rt_OneStep(void)
{
  ...
  /* Check for overrun */
  if (OverrunFlag) {
    rtmSetErrorStatus(testcode_M, "Overrun");
    return;
  }
...
int_T main(int_T argc, const char *argv[])
{
  ...
  for(int i=0 ; i<taille ;i++){
	    rt_OneStep();
	    inp_tab[i]=testcode_U.u;
		out_tab[i]=testcode_Y.y;
  }

  printf("testcode_Y.y = %f\n",testcode_Y.y);
  
  
  saveData(taille,inp_tab,"u.txt");
  saveData(taille,out_tab,"y.txt");

  printf("Warning: The simulation will run forever. "
         "Generated ERT main won't simulate model step behavior. "
         "To change this behavior select the 'MAT-file logging' option.\n");
  fflush((NULL));
  while (rtmGetErrorStatus(testcode_M) == (NULL)) {
